# RPA Qualibank — Simulação de Liberação de Propostas
...
> ⚠️ **Modo atual: SIMULAÇÃO (dry-run).** O robô **nunca aprova nenhuma proposta de verdade**. Ele só lê os dados e registra, em uma planilha, o que *seria* decidido. Nenhum clique em "Ações" → "Aprovação Supervisor" é executado enquanto `DRY_RUN=True` (padrão).
...
└── resultado_simulacao.xlsx  # Relatório gerado (não versionado)
```
...
- **Valor Líquido ≤ R$ 10.000,00** → proposta *seria aprovada*
...
3. Rodar a simulação:
   ...
   python propostas.py
   ```
4. Abrir `resultado_simulacao.xlsx` — os resultados ficam na aba do ano corrente (ex.: `2026`).
...
| Data e Horário da Aprovação do Supervisor | Simulado pelo robô, preenchido só quando a decisão é "Aprovado" |
...
```mermaid
flowchart TD
    subgraph ROBO["Robô RPA — automático (modo simulação)"]
        ...
        H -- Sim --> I[Simular aprovação<br/>NÃO clica em Ações/Aprovação]
...
- A função `aprovar_proposta_real()` (em `propostas.py`) documenta como seria o clique real de aprovação (Ações → Aprovação Supervisor → observação "Aprovado via RPA"), mas **nunca foi executada nem testada** — falta inclusive mapear o botão final de confirmação.
- Antes de desligar o modo simulação (`DRY_RUN=False`), validar o fluxo de aprovação real em ambiente de homologação e com aprovação formal do time responsável.
...
- O relatório gerado (`resultado_simulacao.xlsx`) contém dados reais de clientes e também não é versionado.
